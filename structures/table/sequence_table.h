...
namespace structures 
{
	...
	inline SequenceTable<K, T>& SequenceTable<K, T>::operator=(const SequenceTable<K, T>& other)
	{
		...
		{
			clear();
			for (TableItem<K, T>* item : *other.list_) // skontroluj si to este raz
			{
	...
	inline T & SequenceTable<K, T>::operator[](const K key)
	{
		...
		{
			throw std::logic_error("operator[] in sequence_table: Key not found!");
		}
	}
	...
	inline const T SequenceTable<K, T>::operator[](const K key) const
	{
		...
		{
			throw std::logic_error("operator[](const K key) in sequence_table: Key not found!");
		}
	...
	inline void SequenceTable<K, T>::insert(const K & key, const T & data)
	{
		...
		{
			throw std::logic_error("insert in sequence_table: Key already present!");
		}
