...
namespace structures
{
	...
	inline T & BinarySearchTree<K, T>::operator[](const K key)
	{
		...
		if (found)
		{			//data v node(table item( data v table item )		
			return data = node->accessData()->accessData();
		}
		else
		{
			throw std::out_of_range("BinarySearchTree<K, T>::operator[]: key is not present.");
		}
	}
	...
	{
		BSTTreeNode* node = this->binaryTree_->getRoot();
		if (node == nullptr)
		{
			found = false;
			return nullptr;
		...
		while (nodeKey != key) {
			...
			nodeKey = node->accessData()->getKey();
		}
		found = true;
		return node;
	}
