...
namespace structures
{
	...
	void PriorityQueueLimitedSortedArrayList<T>::push(const int priority, const T & data)
	{
		...
		{
			PriorityQueueSortedArrayList<T>::push(priority, data); // vsetky potrebne atributy mame uz v predchadzajucich strrukturach, ktore dedime.
		}
		else {
			throw std::exception("Limitovany prioritny front je plny");
		}
	}
	...
	{
		PriorityQueueSortedArrayList<T>::push(priority, data); // pushnem 
		if (PriorityQueueList<T>::list_->size() >= capacity_) // ked je velkost listu vacsia nez kapacita tak 
															  //zoberiem prvy  a removnem ho, inak sa len pushne lebo mam dost miesta v kapacite
		{
			return PriorityQueueList<T>::list_->removeAt(0);
		}
		else
		{
			return nullptr;
	...
	template<typename T>
	inline int PriorityQueueLimitedSortedArrayList<T>::minPriority() const // vrati prioritu prvku na indexe 0 (najhorsi prvok).
	{
	...
	{
		if (capacity > PriorityQueueList<T>::list_->size() && capacity > capacity_) //Nova kapacita sa nastavy iba vtedy ak je vascia ako velkost zoznamu
		{
			capacity_ = capacity;
			return true;
		}
