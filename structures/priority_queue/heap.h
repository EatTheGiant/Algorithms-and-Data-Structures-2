...
namespace structures
{
	...
	void Heap<T>::push(const int priority, const T& data)
	{
		...
		int itemIndex = static_cast<int>(PriorityQueueList<T>::list_->size()) - 1;
		int parentIndex = this->getParentIndex(itemIndex);

		while (itemIndex > 0 && item->getPriority() < (*PriorityQueueList<T>::list_)[parentIndex]->getPriority())
		{
			DSRoutines::swap<PriorityQueueItem<T>*>((*PriorityQueueList<T>::list_)[itemIndex], (*PriorityQueueList<T>::list_)[parentIndex]);
	...
	T Heap<T>::pop()
	{
		...
		PriorityQueueItem<T>* removed = (*PriorityQueueList<T>::list_)[this->indexOfPeek()];
		(*PriorityQueueList<T>::list_)[0] = PriorityQueueList<T>::list_->removeAt(static_cast<int>(PriorityQueueList<T>::list_->size()) - 1);

		int itemIndex = 0;
		int sonIndex = this->getGreaterSonIndex(itemIndex);

		while (sonIndex <  PriorityQueueList<T>::list_->size() && (*PriorityQueueList<T>::list_)[itemIndex]->getPriority() > (*PriorityQueueList<T>::list_)[sonIndex]->getPriority())
		{
			DSRoutines::swap<PriorityQueueItem<T>*>((*PriorityQueueList<T>::list_)[itemIndex], (*PriorityQueueList<T>::list_)[sonIndex]);
			itemIndex = sonIndex;
	...
	inline int Heap<T>::getGreaterSonIndex(const int index)
	{
		...
		PriorityQueueItem<T>* lSon = lSonIndex < PriorityQueueList<T>::list_->size() ? (*PriorityQueueList<T>::list_)[lSonIndex] : nullptr;
		PriorityQueueItem<T>* rSon = rSonIndex < PriorityQueueList<T>::list_->size() ? (*PriorityQueueList<T>::list_)[rSonIndex] : nullptr;
		if (rSon == nullptr)
		{
			return lSonIndex;
		}
