...
namespace structures
{
	...
	inline int PriorityQueueList<T>::indexOfPeek() const
	{
		...
		int minValue = (*this->list_)[0]->getPriority();
		int minIndex = 0;
		int i = 0;
		for (PriorityQueueItem<T>* item : *this->list_)
		{
			if (item->getPriority() < minValue)
			{
				minValue = item->getPriority();
				minIndex = i;
			}
			i++;
		}
	...
	{
		int index = this->indexOfPeek();
		//cout << "Priorita " << (*this->list_)[index]->getPriority() << endl;
		return (*this->list_)[index]->getPriority();
	}
}